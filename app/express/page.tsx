...
export default function ExpressPage() {
  ...
  return (
    <>
      ...
      <section className="section-space bg-surface"><div className="site-container grid gap-5 lg:grid-cols-3">{benefits.map(([Icon, title, text], index) => <article key={title} className={`operational-card p-7 reveal-delay-${index + 1}`} data-reveal><Icon className="size-7 text-action-red" aria-hidden="true" /><span className="mt-10 block font-mono text-[10px] font-bold tracking-[.18em] text-muted">0{index + 1} / 03</span><h2 className="mt-4 text-2xl font-bold text-ink">{title}</h2><p className="mt-3 text-sm leading-6 text-muted">{text}</p></article>)}</div></section>
      <section className="section-space bg-white"><div className="site-container"><SectionHeading eyebrow="Como funciona" title="Da solicitação à confirmação da entrega" description="Um fluxo simples para sua empresa iniciar e acompanhar o atendimento." /><ol className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{steps.map((step, index) => <li key={step} className={`process-step p-6 reveal-delay-${index + 1}`} data-reveal><span className="text-5xl font-bold tracking-[-0.05em] text-[#dce9df]">0{index + 1}</span><h2 className="mt-5 text-lg font-bold text-ink">{step}</h2><CheckCircle2 className="absolute right-5 top-5 size-5 text-tracking-green" aria-hidden="true" /></li>)}</ol><p className="mt-6 flex items-start gap-3 rounded-md bg-surface p-4 text-sm leading-6 text-muted"><Truck className="mt-0.5 size-5 shrink-0 text-tracking-green" aria-hidden="true" />Prazos dependem da cidade, rota, tipo de carga e disponibilidade operacional. Consulte nossa equipe.</p></div></section>
      <section className="section-space bg-forest text-white"><div className="site-container flex flex-col items-start justify-between gap-8 md:flex-row md:items-center"><div><p className="eyebrow text-[#8ee49b]">Atendimento Express</p><h2 className="max-w-2xl text-3xl font-bold tracking-[-0.035em] sm:text-4xl lg:text-[2rem]">Consulte agora a disponibilidade para sua rota.</h2></div><QuoteLink size="lg" /></div></section>
    </>
  );
}
