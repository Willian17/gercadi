...
export default function PartnerPage() {
  return (
    <>
      ...
      ].map(([Icon, title, text], index) => { const ItemIcon = Icon as typeof Handshake; return <article key={String(title)} className={`operational-card p-6 reveal-delay-${index + 1}`} data-reveal><ItemIcon className="size-6 text-action-red" /><span className="mt-8 block font-mono text-[10px] font-bold tracking-[.18em] text-muted">0{index + 1} / 04</span><h2 className="mt-4 text-xl font-bold text-ink">{String(title)}</h2><p className="mt-3 text-sm leading-6 text-muted">{String(text)}</p></article>; })}</div></div></section>
      <section className="section-space bg-white"><div className="site-container"><SectionHeading eyebrow="Requisitos" title="O que é necessário para iniciar a conversa" description="A documentação e os requisitos podem variar conforme o modelo da parceria e passarão por avaliação da Gercadi." /><div className="mt-12 grid gap-5 lg:grid-cols-2">{[[FileCheck2, "Documentação necessária", documents], [Network, "Requisitos técnicos", technical]].map(([Icon, title, list], index) => { const ItemIcon = Icon as typeof FileCheck2; return <article key={String(title)} className={`operational-card p-7 sm:p-9 reveal-delay-${index + 1}`} data-reveal><ItemIcon className="size-7 text-tracking-green" /><span className="mt-10 block font-mono text-[10px] font-bold tracking-[.18em] text-muted">0{index + 1} / 02</span><h2 className="mt-4 text-2xl font-bold text-ink">{String(title)}</h2><ul className="mt-6 grid gap-3">{(list as string[]).map((item) => <li key={item} className="flex gap-3 text-sm text-muted"><CheckCircle2 className="size-5 shrink-0 text-tracking-green" />{item}</li>)}</ul></article>; })}</div></div></section>
      <section className="section-space bg-forest text-white"><div className="site-container flex flex-col items-start justify-between gap-8 md:flex-row md:items-center"><div><p className="eyebrow text-[#8ee49b]">Parcerias</p><h2 className="text-3xl font-bold tracking-[-0.035em] sm:text-4xl lg:text-[2rem]">Envie seu interesse para nossa equipe comercial.</h2></div><Button asChild variant="light" size="lg"><a href={`mailto:${siteConfig.email}?subject=Quero ser parceiro da Gercadi`}><FileCheck2 className="size-5" />Enviar e-mail</a></Button></div></section>
    </>
  );
}
