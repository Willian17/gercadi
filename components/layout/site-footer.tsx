    ...
    <footer className="route-grid bg-carbon-deep text-white">
      <div className="site-container border-b border-white/15 py-16 sm:py-24">
        <div className="grid gap-12 lg:grid-cols-12 lg:items-end">
          <div className="lg:col-span-7" data-reveal><p className="text-[10px] font-extrabold uppercase tracking-[.24em] text-white/50">Gercadi Transportes e Logística</p><h2 className="display-condensed mt-6 text-[clamp(3.5rem,6.5vw,6rem)] uppercase lg:text-[clamp(3.5rem,5.5vw,5rem)]">A próxima rota<br /><span className="text-white/30">começa aqui.</span></h2></div>
      ...
      <div className="site-container grid gap-12 py-14 md:grid-cols-2 lg:grid-cols-12">
        ...
        <div className="lg:col-span-3 lg:col-start-6"><h2 className="footer-title">Navegação</h2><ul className="mt-6 grid grid-cols-2 gap-x-6 gap-y-3 text-sm text-white/65">{mainNavigation.map((item)=><li key={item.href}><Link href={item.href} className="hover:text-white">{item.label}</Link></li>)}</ul></div>
