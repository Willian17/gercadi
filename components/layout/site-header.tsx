...
export function SiteHeader() {
  ...
  useEffect(() => {
    ...
    const closeOnOutsideClick = (event: PointerEvent) => {
      if (!servicesMenuRef.current?.contains(event.target as Node)) setServicesMenuPath(null);
    };
  ...
  const navClass = (active: boolean) => cn("nav-link", active && "nav-link-active");
          ...
          >
            <Link href="/" aria-label="Gercadi — página inicial" className="shrink-0 rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-action-red">
              <Image
                ...
                width={1920}
                height={594}
                className={cn("h-auto w-[142px] transition-[width] duration-200 sm:w-[160px]", scrolled && "md:w-[148px]")}
                priority
              />
            </Link>
            ...
            <nav aria-label="Navegação principal" className="hidden items-center gap-0.5 xl:flex">
              {desktopPrimary.slice(0, 3).map((item) => (
                <Link key={item.href} href={item.href} aria-current={isActive(item.href) ? "page" : undefined} className={navClass(isActive(item.href))}>{item.label}</Link>
              ))}
              <div className="relative" ref={servicesMenuRef}>
                <button
                  ...
                  aria-haspopup="menu"
                  onClick={() => setServicesMenuPath((openPath) => openPath === pathname ? null : pathname)}
                >
                  Serviços
                  <ChevronDown className={cn("size-4 transition-transform duration-200", servicesOpen && "rotate-180")} aria-hidden="true" />
                </button>
                <div
                  role="menu"
                  aria-label="Serviços"
                ...
                >
                  {serviceNavigation.map((item, index) => (
                    <Link
                      ...
                      className={cn("flex items-center gap-3 rounded-lg px-3 py-3 text-sm font-bold text-ink transition-colors hover:bg-ivory hover:text-action-red", isActive(item.href) && "bg-ivory text-action-red")}
                    >
            ...
            <div className="hidden shrink-0 items-center gap-1.5 md:flex">
              <TrackingLink size="sm" variant="ghost" className="px-3">Rastrear</TrackingLink>
                ...
                <nav aria-label="Navegação móvel" className="mt-8 flex flex-col">
                  {mainNavigation.map((item, index) => (
                    <SheetClose asChild key={item.href}>
                      <Link href={item.href} aria-current={isActive(item.href) ? "page" : undefined} className={cn("group flex min-h-14 items-center justify-between border-b border-border text-lg font-bold text-ink transition-colors hover:text-action-red", isActive(item.href) && "text-action-red")}>
                        <span>{item.label}</span><span className="font-mono text-[10px] text-muted">0{index + 1}</span>
                      </Link>
                    </SheetClose>
                  ))}
                  <SheetClose asChild><Link href="/seja-parceiro" aria-current={isActive("/seja-parceiro") ? "page" : undefined} className={cn("flex min-h-14 items-center justify-between border-b border-border text-lg font-bold text-ink transition-colors hover:text-action-red", isActive("/seja-parceiro") && "text-action-red")}><span>Seja Parceiro</span><span className="font-mono text-[10px] text-muted">08</span></Link></SheetClose>
                </nav>
